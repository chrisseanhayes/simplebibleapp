...
{
    class DirectPathResolver : IXmlPathResolver
    {
        public string GetPath()
        {
            return "/Users/chrishayes/Documents/code/simplebibleapp/src/simplebibleapp/simplebibleapp/Data/Bible";
        }
    }
    ...
    public class DbGenerator
    {
        ...
        public void BuildSqliteDatabase()
        {
            ...
            var pathresolver = new DirectPathResolver();
            var dbPath = Path.Combine(pathresolver.GetPath(), "bible.db");
            Console.WriteLine($"Database will be built at: {dbPath}");
            ...
            using (var conn = new SqliteConnection(connString))
            {
                ...
                // Insert Books
                var otBooks = File.ReadAllLines(Path.Combine(pathresolver.GetPath(), "OTBookNames.txt"))
                    .Select(line => line.Split('-'))
                    .Select(parts => new { Name = parts[0].Trim(), Abbr = parts[1].Trim(), Testament = "Old" });

                var ntBooks = File.ReadAllLines(Path.Combine(pathresolver.GetPath(), "NTBookNames.txt"))
                    .Select(line => line.Split('-'))
                ...
                Console.WriteLine("Database building completed successfully!");
            }
        }
    }
}
